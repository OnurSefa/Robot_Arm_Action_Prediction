...
def train(epoch=30, learning_rate=0.01, batch_size=256, save_dir='models'):
    ...
    loader = Loader(method='mlp', data_dir="dataset", file_count=10)
    ...
    for e in range(epoch):
        ...
        total_count = 0
        model.train()
        for actions, positions, _ in train_loader:
            optimizer.zero_grad()
        ...
        output_path = f'{save_dir}/mlp_{e}.pt'
        ...
        with torch.no_grad():
            total_loss = 0
            total_count = 0
            for actions, positions, _ in eval_loader:
                actions = actions.to(device)
                positions = positions.to(device)
                predictions = model(actions)
                loss = loss_function(predictions, positions)
    ...
    shutil.copy2(best_path, 'hw1_1.pt')
    ...
    plt.title(f"MLP Model Training Loss Graph")
    plt.legend()
    plt.savefig(f"mlp_training.png", dpi=300)


def test(model_path='hw1_1.pt', batch_size=256, example_count=5):
    ...
    with torch.no_grad():
        ...
        total_count = 0
        current_count = 0
        for actions, positions, imgs_after in test_loader:
            actions = actions.to(device)
            positions = positions.to(device)
            predictions = model(actions)
            loss = loss_function(predictions, positions)
    ...
    plt.tight_layout()
    plt.savefig(f'mlp_evaluation.png', dpi=300)


if __name__ == "__main__":
    train(200, learning_rate=0.002)
    test('hw1_1.pt', example_count=10)
